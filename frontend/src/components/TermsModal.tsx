...
export function TermsModal({ isOpen, onClose, onAccept }: TermsModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div
                ref={modalRef}
                className="bg-[#0f172a] border border-slate-700/50 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200"
                role="dialog"
                aria-modal="true"
                aria-labelledby="modal-title"
            >
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-[#0f172a]/95 sticky top-0 z-10">
                    ...
                    <button
                        onClick={onClose}
                        className="text-slate-400 hover:text-white transition-colors p-2 hover:bg-white/5 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-400"
                ...
                {/* Body (Scrollable) */}
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6 text-slate-300 text-sm leading-relaxed">
                    ...
                    <section className="space-y-3">
                        ...
                        <p>Aplicamos estrictamente el principio de minimización de datos (Art. 5.1.c RGPD). Solo recopilamos:</p>
                        ...
                        <div className="mt-2 p-3 bg-red-500/5 border border-red-500/10 rounded-lg text-red-200/80 text-xs">
                    ...
                    <section className="space-y-3">
                        ...
                        <p>La Aplicación utiliza modelos de Inteligencia Artificial (IA) para clasificar transacciones y generar recomendaciones financieras automáticas.</p>
                        <ul className="list-disc pl-5 space-y-1 text-slate-400">
                    ...
                    <section className="space-y-3">
                        <h3 className="text-lg font-medium text-white flex items-center gap-2">
                            <span className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-xs text-slate-400">4</span>
                    ...
                    <section className="space-y-3">
                        <h3 className="text-lg font-medium text-white flex items-center gap-2">
                            <span className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-xs text-slate-400">5</span>
                            Tus Derechos (Derechos ARCO y RGPD)
                        </h3>
                        <p>Tienes en todo momento el control total sobre tus datos. A través del apartado "Privacidad" en la Aplicación puedes ejercer tus derechos de:</p>
                        <ul className="list-disc pl-5 space-y-1 text-slate-400">
                ...
                {/* Footer */}
                <div className="p-6 border-t border-slate-800 bg-[#0f172a]/95 flex items-center justify-between gap-4">
